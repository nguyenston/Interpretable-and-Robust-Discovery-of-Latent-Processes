...
\section{Proofs for Method of moments for HIL}
\subsection{\cref{lem:actionProjection}}
\paragraph{Statement:}
let $\bm{V}_s\in\mathbb{R}^{\alpha\times\omega}$ for $s \in \mathcal{S}$ be a matrix of right singular vectors corresponding to the $\omega$ largest singular values of $\bm{K}_s$.
We then have the following lemma.
Define the block-diagonal matrices
\[\label{eq:Vmat_restate}
...
\[
    ...
    &=\lrc{\bm{\Xi}_s\bm{\Pi}^{hi}_s\bm{\Pi}^{lo}_s}[s_1,a_2].\label{eq:overline_K_derive_restate}
\]
...
\]
Finally, we can write Equation \eqref{eq:diagonalize_restate} as
\[
...
\subsection{\cref{thm:find_pi_hi}}
\paragraph{Statement:}
\[\label{eq:pihi_recovery_restate}
...
\begin{itemize}
    ...
    \[\label{eq:hat_K_definition_restate}
        \bm{\hat{K}}_{ss'}[s'',a]=\bm{\hat{M}}[s\zeta+s'',s'\alpha+a].
    \]
    \item $\bm{w}_{s'}$ are length $\zeta$ weight vectors of choice subject to
    \[\label{eq:pihi_weight_constraint_restate}
        \bm{w}_i\transpose\bm{\Psi} \unit_i\transpose=1,\ \forall i\in\mathcal{S}.
    \]
\end{itemize}

\begin{proof}
According to Definition \eqref{eq:hat_K_definition_restate} and Equation \eqref{eq:overline_K_derive_restate}, $\bm{K}_s$ and $\bm{\hat{K}}_{ss'}$ can be written as following:
\[
...
% \hspace*{\fill} \qed
\end{proof}
\section{Order recovery process for method of moments}\label{sec:order-recovery}
From Equation \eqref{eq:diagonalize_detailed}, we know the eigenbasis $\bm{B}\in\mathbb{R}^{\zeta\omega\times\zeta\omega}$ is of the form
\[\label{eq:true_basis}
    \bm{B} = \lrp{\bm{\Pi}^{hi}\bm{\Pi}^{lo}\bm{V}}\inv
        \lrp{\bm{\Psi}\inv\otimes\identity_\omega}.
\]
The eigen-decomposition \eqref{eq:diagonalize} will introduce an unknown scaling and permutation to the columns of the basis:
\[\label{eq:shuffled_basis}
    \bm{\hat{B}} = \bm{B}\mathrm{diag}(\bm{c})\bm{\mathcal{P}},
\]
where $\bm{c}$ is a vector that corresponds to the scaling of each column, and $\bm{\mathcal{P}}$ is a permutation operator on the columns.

For convenience, define the following shorthands:
\[
    \bm{X}_s = \lrp{\bm{\Pi}^{hi}_s\bm{\Pi}^{lo}_s\bm{V}_s}\inv,
\]
\[
    \bm{X} = \lrp{\bm{\Pi}^{hi}\bm{\Pi}^{lo}\bm{V}}\inv = \mathrm{diag}(\bm{X}_1, \ldots, \bm{X}_\zeta),
\]
and $\bm{\Gamma}=\bm{\Psi}\inv$ with elements $\gamma_{ij}$. %=\bmat{\gamma_{11}&\dots&\gamma_{1\zeta}\\
%         \vdots&\ddots&\vdots\\
%         \gamma_{\zeta 1}&\dots&\gamma_{\zeta\zeta}}.
% \]
Rewrite \eqref{eq:true_basis} as
$
    \bm{B} =\bm{X}\left(\bm{\Gamma}\otimes\identity_\omega\right)
    $. 
    % &=\bmat{\bm{X}_1&&\\&\ddots&\\&&\bm{X}_\zeta}
    %     \bmat{\gamma_{11}\identity_\omega&\dots&\gamma_{1\zeta}\identity_\omega\\
    %     \vdots&\ddots&\vdots\\
    %     \gamma_{\zeta 1}\identity_\omega&\dots&\gamma_{\zeta\zeta}\identity_\omega}\\
%     =\bmat{\gamma_{11}\bm{X}_1&\dots&\gamma_{1\zeta}\bm{X}_1\\
%         \vdots&\ddots&\vdots\\
%         \gamma_{\zeta 1}\bm{X}_\zeta&\dots&\gamma_{\zeta\zeta}\bm{X}_\zeta}. %\numberthis\\
% \]
Recalling Equation \eqref{eq:shuffled_basis}, we have that
% \[
%     \bm{\hat{B}}=
%         \bmat{\gamma_{11}\bm{X}_1&\dots&\gamma_{1\zeta}\bm{X}_1\\
%         \vdots&\ddots&\vdots\\
%         \gamma_{\zeta 1}\bm{X}_\zeta&\dots&\gamma_{\zeta\zeta}\bm{X}_\zeta}
%         \mathrm{diag}(\bm{c})\bm{\mathcal{P}}.
% \]
the structure of the sub-matrices $\bm{\hat{J}}_s\in\mathbb{R}^{\omega\times\zeta\omega}$
of the basis $\bm{\hat{B}}$ is 
$
    \bm{\hat{J}}_s=\bmat{\gamma_{s1}\bm{X}_s&\dots&\gamma_{s\zeta}\bm{X}s}
        \mathrm{diag}(\bm{c})\bm{\mathcal{P}}.
$
It is easily seen that each of these sub-matrices contains $\omega$ sets of $\zeta$ linearly dependent vectors, and that the grouping of these linearly dependent columns are identical due to them sharing the same permutation $\bm{\mathcal{P}}$. We separate and represent these sets as $\zeta\omega\times\zeta$ matrices $\bm{\hat{Q}}_o$ given by
\[\label{eq:sub_shuffled_basis}
    \bm{\hat{Q}}_o=
        \bmat{\gamma_{11}\bm{X}_1\unit_o&\dots&\gamma_{1\zeta}\bm{X}_1\unit_o\\
        \vdots&\ddots&\vdots\\
        \gamma_{\zeta 1}\bm{X}_\zeta\unit_o&\dots&\gamma_{\zeta\zeta}\bm{X}_\zeta\unit_o}
        \mathrm{diag}(\bm{c}_o)\bm{\mathcal{P}}_{o},
\]
where $\bm{c}_o$ and $\bm{\mathcal{P}}_o$ are unknown scaling factor and permutation corresponding to the group $o$.

We define
$
    \bm{d}_o=\bmat{\bm{X}_1\transpose\unit_o\transpose &
        \dots & \bm{X}_\zeta\transpose\unit_o\transpose}\transpose \in\mathbb{R}^{\zeta\omega}.
$ Then, \eqref{eq:sub_shuffled_basis} can be rewritten as 
$
    \bm{\hat{Q}}_o=\mathrm{diag}(\bm{d}_o)(\bm{\Gamma}\otimes\one_\omega)
        \mathrm{diag}(\bm{c}_o)\bm{\mathcal{P}}_o.
$
In order to recover the original ordering of the columns of $\bm{\hat{Q}}_o$, we need to somehow match them with the columns of $\bm{\Gamma}\otimes\one_\omega$, which is a known quantity.

Before proceeding, let us introduce the element-wise inverse operator $\oslash$ such that $\bm{A}^\oslash$ corresponds to the matrix formed by inverting each element of the matrix $\bm{A}$, and so that
% \[
%     \bm{A}^\oslash=\bmat{a_{11}\inv&\dots&a_{1n}\inv\\\vdots&\ddots&\vdots\\a_{m1}\inv&\dots&a_{mn}\inv},
% \]
$
    (\bm{A}\bm{\mathcal{P}})^\oslash=\bm{A}^\oslash\bm{\mathcal{P}},
$ and
$
    [\mathrm{diag}(\bm{u})\bm{A}\mathrm{diag}(\bm{v})]^\oslash=\mathrm{diag}(\bm{u})\inv\bm{A}^\oslash\mathrm{diag}(\bm{v})\inv.%\numberthis
$
Let's assume that $\bm{\Gamma}\otimes\one_\omega$ has no zero entries. If there are zero entries, we can further partition $\bm{\hat{Q}}_o$ and $\bm{\Gamma}\otimes\one_\omega$ into column groups that has the same rows with zero entries, remove those rows, and use the following procedure on each of the groups before combining the result.
%{\color{red} Does this need more elaboration?}

We have the following reduction:
\[
    \bm{\hat{Q}}_o^\oslash\bm{\hat{Q}}_o\transpose
    % =\ &\mathrm{diag}(\bm{d}_o)\inv(\bm{\Gamma}^\oslash\otimes\one_\omega)
    %     \mathrm{diag}(\bm{c}_o)\inv\bm{\mathcal{P}}_o \\
    % &\phantom{\mathrm{diag}}\bm{\mathcal{P}}_o\transpose\mathrm{diag}(\bm{c}_o)
    %     (\bm{\Gamma}\transpose\otimes\one_\omega)\mathrm{diag}(\bm{d}_o) \\
    =\ &\mathrm{diag}(\bm{d}_o)\inv(\bm{\Gamma}^\oslash\bm{\Gamma}\transpose
        \otimes\one_\omega\one_\omega\transpose)\mathrm{diag}(\bm{d}_o) \\
    =\ &(\bm{d}_o^\oslash\bm{d}_o\transpose)\circ
        (\bm{\Gamma}^\oslash\bm{\Gamma}\transpose\otimes\one_\omega\one_\omega\transpose).%\numberthis
\]
Therefore
$
    \bm{\hat{Q}}_o^\oslash\bm{\hat{Q}}_o\transpose\circ
        (\bm{\Gamma}^\oslash\bm{\Gamma}\transpose\otimes\one_\omega\one_\omega\transpose)^\oslash
        =\bm{d}_o^\oslash\bm{d}_o\transpose.
$
It is easily verifiable that $\mathrm{diag}(\bm{d}_o^\oslash\bm{d}_o\transpose\unit_i)\mathrm{diag}(\bm{d}_o)=\bm{d}_o[i]\identity$.

We have $\mathrm{diag}(\bm{d}_o^\oslash\bm{d}_o\transpose\one_{\zeta\omega})\bm{\hat{Q}}_o=
        (\one_{\zeta\omega}\transpose\bm{d}_o)(\bm{\Gamma}\otimes\one_\omega)
        \mathrm{diag}(\bm{c}_o)\bm{\mathcal{P}}_o$. Notice that the columns of this matrix are just multiples of the columns of $(\bm{\Gamma}\otimes\one_\omega)$. As such a matching can be computed by checking linear dependence between the columns of the two matrices.
\section{Robust Consistency of \methodname} 
\label{sec:theory}


We now show that, under reasonable assumptions, \methodname is robustly consistent for probabilistic matrix factorization.
Due to the complex dependence structures in probabilistic matrix factorization, we limit 
...
We will develop a general theory, then discuss how \cref{thm:pmf-robust-consistency} follows 
as a corollary.
% Proofs of all mixture model-related results are in \cref{sec:PMF-consistency-proof}. 
...
metric, maximum mean discrepancy, and 1-Wasserstein distance -- are jointly convex.
...
well-behaved.
For example, for the Kullback--Leibler divergence estimators
...
\cref{assump:metric-discr-conditions}(e) is not overly restrictive.
% and we discuss some relevant examples in \cref{sec:divergence-choice} below.
...
This should holds for most applications under mild conditions
...
The conclusion follows immediately from these two results.
...
The following lemma states that integral probability metrics (as defined in Eq.\ (8))\PROBLEM{WHAT IS THIS?} are jointly convex -- that is, they satisfy \cref{assump:metric-discr-conditions}(b).
...
In \cref{sec:intro}\PROBLEM{WHAT IS THIS?} , we consider the case of two clusters of equal size. 
...
\begin{figure}[]
  \centering
  \subfloat[Well specified]{\includegraphics[width=0.9\textwidth]{figures/composite-multdiv-600-breast-custom-seed-1.pdf}}\\
...
\begin{figure}[]\ContinuedFloat
  \centering
  \subfloat[Contaminated]{\includegraphics[width=0.9\textwidth]{figures/composite-multdiv-600-breast-custom-seed-1-contamination-2.pdf}}\\
  \caption{Estimation quality for various scheme of data generation. See \cref{fig:mutsig_result} caption for explanation.}
...
\begin{figure}[]\ContinuedFloat
  \centering
  \subfloat[Overdispersed]{\includegraphics[width=0.9\textwidth]{figures/composite-multdiv-600-breast-custom-seed-1-overdispersed-2.0.pdf}}
  \caption{Estimation quality for various scheme of data generation. See \cref{fig:mutsig_result} caption for explanation.}
...
\begin{figure}[]\ContinuedFloat
  \centering
  \subfloat[Perturbed]{\includegraphics[width=0.9\textwidth]{figures/composite-multdiv-600-breast-custom-seed-1-perturbed-0.0025.pdf}}
  \caption{Estimation quality for various scheme of data generation. See \cref{fig:mutsig_result} caption for explanation.}
...
\begin{figure}[h!]
  ...
  \subfloat[Unlabeled (left) and labeled (right) versions of the urban dataset]{\includegraphics[width=0.9\textwidth]{./figures/urban_gt_5endmembers.pdf}}\\
	\subfloat[$K=3$]{\includegraphics[width=0.44\textwidth]{figures/urban_K=3.pdf}}
	\subfloat[$K=4$]{\includegraphics[width=0.44\textwidth]{figures/urban_K=4.pdf}}\\
	\subfloat[$K=5$]{\includegraphics[width=0.44\textwidth]{figures/urban_K=5.pdf}}
	\subfloat[$K=6$]{\includegraphics[width=0.44\textwidth]{figures/urban_K=6.pdf}}
